...
void displayFast_Log(void){
      ...
      Display_Line6  = " TOUCH UP KEY 5 TIMES";   
      Display_Line7  = "TO END  DATA LOGGING!";    
...
void displayValues(void){
  ...
  if(Loop.FastLog_Init == ON)displayFast_Log();
...
void UpdateDisplayMenu(void) {
  ...
  switch (MainMenu) {
    ...
    case MENU1_SUB2 :     str = CopyFlashToRam(Disp_MENU1_SUB2); //   str +=CopyFlashToRam(Disp_MENU1_SUB2);
      ...
      break;
    case MENU1_SUB3 : 
              if(Loop.FastLog_Init == ON)str = CopyFlashToRam(Disp_MENU1_SUB3_FAST);
              else str = CopyFlashToRam(Disp_MENU1_SUB3) ;//str +=CopyFlashToRam(Disp_MENU1_SUB3);
      break;
    ...
    case MENU2_SUB7 : //str = CopyFlashToRam(Disp_MENU2_SUB);
      ...
      break;
    case MENU2_SUB8 :// str = CopyFlashToRam(Disp_MENU2_SUB);
      if(SampleTime == TASK_2SEC)str = CopyFlashToRam(Disp_MENU_Activ);
      else str = CopyFlashToRam(Disp_MENU_Enter);
      str += CopyFlashToRam(Disp_MENU2_SUB8);
      if(SampleTime != TASK_2SEC)       
        if (DisplayFlash() == OFF)str[10] = ' ';         
      
      break;
    case MENU2_SUB9 : //str = CopyFlashToRam(Disp_MENU2_SUB);
      if(SampleTime == TASK_5SEC)str = CopyFlashToRam(Disp_MENU_Activ);
      else str = CopyFlashToRam(Disp_MENU_Enter);  
      str += CopyFlashToRam(Disp_MENU2_SUB9);
