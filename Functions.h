...
void Common_Loop(){
  if (Loop.Task_10msec) {
    Loop.Task_10msec = OFF;
    if ((SampleTime == TASK_10MSEC) && (Loop.FastLog_Status == ON))Log_Data_Write_SD();
  }
  if (Loop.Task_20msec) {
    Loop.Task_20msec = OFF;
    if ((SampleTime == TASK_20MSEC) && (Loop.FastLog_Status == ON)) Log_Data_Write_SD();  
  }
  if (Loop.Task_50msec) {
    Loop.Task_50msec = OFF;
    if ((SampleTime == TASK_50MSEC) && (Loop.FastLog_Status == ON)) Log_Data_Write_SD();
  }
  if (Loop.Task_100msec) {
    Loop.Task_100msec = OFF;
    if ((SampleTime == TASK_100MSEC) && (Loop.FastLog_Status == ON))Log_Data_Write_SD();    
  }
  if (Loop.Task_250msec) {
    Loop.Task_250msec = OFF;
    if ((SampleTime == TASK_250MSEC) && (Loop.FastLog_Status == ON))Log_Data_Write_SD();
    
    if(Loop.FastLog_Status == OFF){
  ...
  if (Loop.Task_500msec) {
    ...
    if ((SampleTime == TASK_500MSEC) && (Loop.FastLog_Status == ON)){
      Log_Data_Write_SD();
    }
    if((Loop.FastLog_Init == ON) && (Loop.FastLog_Status == OFF)){  
      if(Loop.FastLog_DelayTimer)Loop.FastLog_DelayTimer--;
      else{
          Loop.FastLog_Init = OFF;
          Loop.FastLog_Status = ON;    
          SDCard.LogEnable = ON; 
  ...
  if (Loop.Task_1Sec) {
    ...
    if(Loop.FastLog_Status == OFF)SerialPortRx();
    if(Loop.FastLog_Status == OFF)RTC_TimeClock();
  ...
  if(Loop.FastLog_Status == OFF){
    ...
    if (SampleTime == TASK_1SEC)Log_Data_Write_SD(); 
     ...
     if((MainMenu == MENU5_SUB7)||(MainMenu == MENU2_SUB14)||(MainMenu == MENU3_SUB3) ||(MainMenu == MENU3_SUB4)||(MainMenu == MENU1_SUB3)
  ...
  if (Loop.Task_5Sec) {
    ...
    if (SampleTime == TASK_5SEC) Log_Data_Write_SD();
          ...
          if(Loop.FastLog_Status == OFF) SDS_DustSensor();
      #endif
   // if(Loop.FastLog_Status == OFF)Relay_loop(); 
  }
  #if defined (ARDUINO_MEGA) | defined (ARDUINO_DUE) 
  if (Loop.Task_10Sec) {
    Loop.Task_10Sec = OFF;
