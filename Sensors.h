...
void SensorACccel_GyroInit(){
  ...
  /*  
    ...
    Serial.print(lsm.gyroscopeSampleRate());
    Serial.println(F(" Hz"));
    Serial.println();
...
void SensorAcccel_GyroRead(){
 ...
 //  float x, y, z;
   ...
   Accelometer.x = a.acceleration.x;
   Accelometer.y = a.acceleration.y;  
   Accelometer.z = a.acceleration.z;
...
/*
  if (lsm.accelerationAvailable()) {
      lsm.readAcceleration(Accelometer.x, Accelometer.y, Accelometer.z);
