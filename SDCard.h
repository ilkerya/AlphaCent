  ...
  if(!SDCard.LogEnable){ // log on of  from menu
   ...
   if(SDCard.PauseTimer){  
    ...
    if (dataFile) {
      ...
      if(Loop.FastLog_Status == OFF) FileSize.Total = dataFile.size();  
              //  Serial.println(dataFile.position()); // 2021.03.09 
              //  Serial.println(FileSize.Total); // 2021.03.09 
     dataFile.close();   
     
      if(Loop.FastLog_Status == ON) {  
        if(DebugState == OFF) {    
          Serial.print(SD_Counter.EndCount);
          Serial.print("  ->  ");     
      ...
      if(Loop.FastLog_Status == OFF) UpdateFileQue(); 
    }      
  else {  
...
#define SPI_1MHZ 1000000// SPI_FULL_SPEED
  ...
  #endif
  #if defined (ARDUINO_MEGA) | defined (ARDUINO_DUE) |  defined (STM32_F407) 
  // if (!card.init(SPI_HALF_SPEED, SD_CS_PINOUT)) {  // SPI_QUARTER_SPEED  // Sd2Card.h  
  //  if (!card.init(SPI_1MHZ, SD_CS_PINOUT)) {  // SPI_QUARTER_SPEED  // Sd2Card.h  SPI_DIV3_SPEED
     if (!card.init(SPI_FULL_SPEED, SD_CS_PINOUT)) {  // SPI_QUARTER_SPEED  // Sd2Card.h  SPI_DIV3_SPEED 
  #endif      
...
void SD_Card_Init(){
    if(Loop.FastLog_Status == OFF) Serial.println(F("SD_Card_Init Start"));
    #ifdef ARDUINO_DUE 
    Print_ARM_SPI_Regs();
    #endif
    #ifdef ARDUINO_MKRZERO 
  ...
  if(Loop.FastLog_Status == OFF)Serial.println(F("SD_Card_Init Success"));
  #ifdef   ARDUINO_DUE     
      if(Loop.FastLog_Status == OFF)Print_ARM_SPI_Regs();
  #endif
}
void SD_Card_Header_Preparation(){
   ...
   switch(SDCard.Status){
    ...
    dataString += String(SensorId.OnBoard, HEX) +  ',' + String(SensorId.No1, HEX) +  ',' +  
        ...
        dataString += F("Bat. Current,Bat. Voltage,");   
      #endif        
...
void SD_Card_Data_Preparation(){
      ...
      #endif   
      #ifdef BATTERY_SCOOTER_EXISTS     
        dataString += String(Values.Battery_Current);
        dataString += ',';       
        dataString +=  String(Values.Battery_Voltage);
       dataString += ',';
      ...
      #endif
      #ifdef   ACCL_GYRO_SENSOR_EXISTS       
         dataString += String(Accelometer.x);   
         dataString += ',';
         dataString += String(Accelometer.y);
         dataString += ',';
         dataString += String(Accelometer.z); 
         dataString += ',';
